...
@section('title','All in One Search')
            ...
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form id="cuelink" action="{{route('admin.allin.store')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="mb-3" >
                                    <label class="form-label required">Select Website/Brand</label>
                                    <select id='testSelect1' name="cam_id[]" class="form-control">
                                        @foreach($campaigns as $partner)
                        ...
                        <form id="impact" action="{{route('admin.allin.store')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="mb-3">
                                    <label class="form-label required">Select Website/Brand</label>
                        ...
                        <div id="table-default" class="table-bordered table-responsive">
                            <table class="table">
                                ...
                                <tbody class="table-tbody">
                                    <tr>
                                        @foreach($all as $key=>$homead)
                                    <tr>
                                        <td id="row">{{$key+1}}</td>
                                        @if($homead->campaign_id)
                                            <td class="sort-name">{{$homead->cuelinkcampaigns->name}}</td>
                                            <td>Cuelink</td>
                                            <td class="sort-name"><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampl2eModal{{$homead->id}}">
                                                    Add/Update</button></td>
                                            <td class="sort-type">
                                                <a href="{{route('admin.allin.delete',[$homead['id']])}}" class="btn btn-danger" onclick="return confirm('Are you sure?');" title="delete heading">Delete
                                                </a>    
                                            </td>
                                       @elseif($homead->admited_id)
										
                                       <td class="sort-name">{{ $homead->admited->name}}</td>
                                       <td>Admited</td>
                                       <td class="sort-name"><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampl2eModal{{$homead->id}}">
                                               Add/Update</button></td>
                                       <td class="sort-type">
                                       ...
                                       @elseif($homead->impact_id)
                                       <td class="sort-name">{{$homead->impact->name}}</td>
                                       <td>Impact</td>
                                       <td class="sort-name"><button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampl2eModal{{$homead->id}}">
                                               Add/Update</button></td>
                                       <td class="sort-type">
                                    ...
                                    <div class="modal fade" id="exampl2eModal{{$homead->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel{{$homead->id}}" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <form action="{{route('admin.allin.update',$homead->id)}}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="example2ModalLabel{{$homead->id}}" align="left">Add Tab Color/search url</h5>
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="text" name="search_url" value="{{$homead->search_url}}" class="form-control">
                                                        </div>
                                                        <div class="mb-3">
                                                            <label class="form-label required">Tab Color</label>
                                                            <input type="color" name="color" value="{{$homead->tab_color}}" class="form-control">
                                                        </div>
