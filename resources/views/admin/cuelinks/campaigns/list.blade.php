...
@section('title','Cuelinks Campaigns List')
    ...
    <div class="container-xl">
        <div class="card">
            <div class="card-header">
                ...
                <a align="right" style="float:right !important;" href="{{route('admin.cuelinkscampaigns.export-all')}}"
                    ...
                    <table id="example" class="display" style="width:100%">
                        ...
                        <tbody class="table-tbody">
                            ...
                            <tr>
                                ...
                                <td class="sort-name"> @if($e['image']!=NULL)<img
                                        src="{{asset('storage/app/public/offer')}}/{{$e['image']}}" alt="no image"
                                ...
                                <td class="sort-name"> @if($e['description']==NULL)
                                    <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampl2eModal{{$e->id}}">Add Description</button>
                                    @else
                                    <p>{{$e['description']}}</p>
                                    @endif
                                </td>
                                ...
                                <td class="sort-name"> {{$e['domain']}}</td>
                                ...
                                <td class="sort-role" align="center">
                                    ...
                                    {
                                        window.location.href = "{{route('admin.cuelinkscampaigns.campagns_status',[$e->id,$e->status?0:1])}}"
                                    }
                                    }
                                    </script>  
                                </td>
                                ...
                                <td class="sort-type">
                                    <a href="{{route('admin.cuelinkscampaigns.delete',$e['id'])}}"
...
<div class="modal fade" id="exampleModal{{$e->id}}" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" role="document">
        <form action="{{route('admin.cuelinkscampaigns.update_cam_img',$e->id)}}" method="post"
...
<div class="modal fade" id="exampl2eModal{{$e->id}}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel{{$e->id}}" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{route('admin.cuelinkscampaigns.update_desc',$e->id)}}" method="post"
            enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="example2ModalLabel{{$e->id}}" align="left">Add Description</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close" align="right">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="description"
                            id="summernote">{{$e['description']}}</textarea>
                    </div>
