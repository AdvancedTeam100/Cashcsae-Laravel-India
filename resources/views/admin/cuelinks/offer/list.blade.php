...
@section('title','Cuelinks Offer List')
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="flex">
                    ...
                    href="{{route('admin.cuelinksoffers.export-all')}}" class="btn btn-danger  float-right">Export All
                </a>&nbsp;
            </div>
            <div class="card-body">
                    ...
                    <table id="example" class="display" style="width:100%">
                        ...
                        <tbody class="table-tbody">
                            ...
                            <tr>
                                ...
                                <td class="sort-name">{{$e['campaign_name']}}</td>
                                <td class="sort-name">{{$e['name']}}</td>
                                <td class="sort-name">
                                    @if($e['banner_image']!=NULL)
                                    <img src="{{asset('storage/app/public/offer')}}/{{$e['banner_image']}}"
                                ...
                                <td class="sort-name">{{$e['start_date']}}</td>
                                <td class="sort-name">{{$e['end_date']}}</td>
                                <td class="sort-name">{{$e['category']}} </td>
                                <td class="sort-role">
                                ...
                                {
                                    window.location.href = "{{route('admin.cuelinksoffers.offer_status',[$e->id,$e->status?0:1])}}"
                                }
                                }
                                </script>  
                                </td>
                                <td class="sort-role">
                                    <center><label class="form-check form-check-single form-switch"
                                            ...
                                            <input type="checkbox" class="form-check-input"
                                                onclick="myFunction{{$e->id}}()" id="stocksCheckbox{{$e->id}}"
                                                {{$e->exclusive?'checked':''}}>
                                        </label>
                                    </center>
                                    <script>
                                    ...
                                    {
                                        window.location.href = "{{route('admin.cuelinksoffers.exclusive',[$e->id,$e->exclusive?0:1])}}"
                                    }
                                    }
                                    </script>
                                ...
                                <td class="sort-type">
                                    <a href="{{route('admin.cuelinksoffers.delete',$e->id)}}" class="btn btn-danger"
...
<div class="modal fade" id="exampleModal{{$e->id}}" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" role="document">
        <form action="{{route('admin.cuelinksoffers.update_offer_img',$e->id)}}" method="post"
            enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel{{$e->id}}" align="left">Add Banner Image</h5>
                    ...
                    <div class="form-group" id="viewerbox" style="display:none;margin-bottom:0%;">
                        <center>
                            <img style="width: 200px;border: 1px solid #3399db; border-radius: 10px; max-height:200px; padding: 11px;"
                                id="viewer" src="{{asset('storage/app/public/offer')}}/{{$e['banner_image']}}"
                                alt="offer banner" />
                        </center>
                    </div>
