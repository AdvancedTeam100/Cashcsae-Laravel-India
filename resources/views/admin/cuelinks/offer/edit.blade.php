                  ...
                  <div class="mb-3">
                            <div class="form-group">
                                ...
                                <select name="category_id" id="category-id" class="form-control js-select2-custom"
                                        onchange="getRequest('{{url('/')}}/offer/get_categories?parent_id='+this.value,'sub-categories')">
                                    <option value="">---select---</option>
                                    @php($categories=\App\Models\Category::where(['position' => 0])->get())
                                    @foreach($categories as $category)
                                        <option value="{{$category['id']}}" {{ $category->id==json_decode($e->category_ids)[0]->id ? 'selected' : ''}} >{{$category['name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                     <div class="mb-3">
                            <div class="form-group">
                                <label class="input-label" for="exampleFormControlSelect1">Sub Category<span
                                        class="input-label-secondary" title="Category Required"></span></label>
  ...
  <script>
        ...
        $(document).ready(function () {
            setTimeout(function () {
                ...
                getRequest('{{url('/')}}/offer/get_categories?parent_id=' + category + '&sub_category=' + sub_category, 'sub-categories');
                getRequest('{{url('/')}}/offer/get_categories?parent_id=' + sub_category + '&sub_category=' + sub_sub_category, 'sub-sub-categories');
